...
#     """
#
#     def _conv_block(self, in_channels, out_channels, dropout: float = 0):
#         conv_layer1 = [
...
#         add_on_layer = []
#         if self.batch_norm:
#             add_on_layer.append(nn.BatchNorm3d(num_features=out_channels))
#         if dropout > 0:
#             add_on_layer.append(nn.Dropout(p=dropout))
#         args = conv_layer1 + add_on_layer + conv_layer2 + add_on_layer
...
#                  depth=4, initial_features=16, decoder_dropout: float = 0.1,
#                  encoder_dropout: float = 0.1, BN: bool = False, elu=False,
...
#                               n_features_encode[level + 1], self.encoder_dropout)
#              for level in range(self.depth)])
#
#         # the base convolution block
#         self.base = self._conv_block(n_features_encode[-1],
#                                      n_features_base, self.encoder_dropout)
#         self.decoder = nn.ModuleList(
#             [self._conv_block(n_features_decode[level],
#                               n_features_decode[level + 1], self.decoder_dropout)
#              for level in range(self.depth)])
#
...
#
#         # apply output conv and activation
#         x = self.out_conv(x)
