...
parser.add_argument("-cv_folds", "--cv_folds", type=int)
...
# Generate cv folds dictionary
# cv_folds = 5
shuffle(tomo_training_list)
cv_sets = np.array_split(tomo_training_list, 5)
cv_training_dict = {}
cv_evaluation_dict = {}
for cv_idx, cv_ids in enumerate(cv_sets):
    cum_cv_set = []
    cv_evaluation_dict[cv_idx] = list(cv_ids)
    for cv_idx_, cv_ids_ in enumerate(cv_sets):
        if cv_idx_ != cv_idx:
            cum_cv_set += list(cv_ids_)
    cv_training_dict[cv_idx] = cum_cv_set

for key in cv_training_dict.keys():
    model_name = args.model_name[:-4] + "_cv" + str(key)
    tomo_training_list = cv_training_dict[key]
    tomo_evaluation_list = cv_evaluation_dict[key]
    print("tomo_training_list", tomo_training_list)
    print("tomo_evaluation_list", tomo_evaluation_list)
    ...
    for tomo_name in tomo_evaluation_list:
        ...
        _, partition_path = training_partition_path(output_dir=work_dir, tomo_name=tomo_name,
                                                    partition_name=partition_name)
        evaluation_partition_paths += [partition_path]
        data_aug_rounds_list += [0]
    ...
    evaluation_loss = validate(model=net, loader=eval_loader, loss_function=loss,
                               metric=metric, device=device, step=0, tb_logger=None)
    performance = 1 - evaluation_loss
    statistics_label = model_name + "_dice"
    if args.statistics_file != "None":
        write_statistics(statistics_file=args.statistics_file,
                         statistics_label=statistics_label,
                         tomo_name="cv_" + str(key),
                         stat_measure=performance)
